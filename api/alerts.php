...
if (!$email || $keyword === '') { http_response_code(422); echo json_encode(['error'=>'Enter a valid email and job keyword.']); exit; }
...
$record = ['email'=>$email,'keyword'=>mb_substr($keyword,0,120),'location'=>mb_substr($location,0,120),'frequency'=>$frequency,'status'=>'active','unsubscribe_token'=>bin2hex(random_bytes(24)),'created_at'=>gmdate('c')];
